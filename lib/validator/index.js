  ...
  validateStruct() {
    ...
    try {
      for (const key of structKeys) {
        ...
        const emptyArray = Array.isArray(req[key]) && req[key].length === 0;
        //check type of value
        if (req[key] !== undefined) {
          if (!struct[key].type.includes(typeof req[key])) {
        ...
        if (struct[key].validate.find((v) => v.includes("required"))) {
          if (emptyArray) throw new Error(domain.validationFailureErrResMsg);
